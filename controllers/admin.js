...
--------------------------------------------
 */
var Dealer, EventProxy, Lots, User, cacheLot, config, crypto, csv, defaultDealer, ejs, exportCSV, fs, getChanged, getCtrl, getDates, getList, getLot, getReser, getType, getlots, gettf, helper, nodeExcel, path, resetCode, str, unique;
...
exports.dealercheck = function(req, res, next) {
  var mobile, re;
  mobile = req.body.mobile;
  re = new helper.recode();
  return User.getUserByMobile(mobile, function(err, user) {
    if (user != null) {
      if (user.dealer !== req.cookies.user) {
...
exports.dealerpage = function(req, res, next) {
  ...
  for (i = _i = 0; 0 <= sortcols ? _i < sortcols : _i > sortcols; i = 0 <= sortcols ? ++_i : --_i) {
    t = parseInt(req.body["iSortCol_" + i]);
    ad = req.body["sSortDir_" + i] === "asc" ? 1 : -1;
    if (t === 0) {
      sortfield.create_at = ad;
    }
    if (t === 1) {
      sortfield.username = ad;
    }
    if (t === 2) {
      sortfield.mobile = ad;
    }
    if (t === 3) {
      sortfield.cartype = ad;
    }
    if (t === 4) {
      sortfield.reser_at = ad;
    }
    if (t === 5) {
      sortfield.imp_at = ad;
    }
    if (t === 6) {
      sortfield.changed = ad;
    }
  }
  return User.getUserByDealer(req.cookies.user, pageStar, pageSize, sortfield, startime, endtime, type, search, function(err, count, resutls) {
    var a, poback, _j, _len, _ref;
    poback = [];
    _ref = unique(resutls);
    ...
    return res.send({
      "mData": pageStar / pageSize + 1,
      "iTotalRecords": poback.length,
...
exports.download = function(req, res, next) {
  ...
  conf.rows = [];
  return User.getUserByDealerAll(req.cookies.user, function(err, results) {
...
getType = function(n) {
  var type;
  if ((n == null) || parseInt(n) > 6 || parseInt(n) < 1 || n === "cartype") {
    return "";
  }
  type = [
    {
...
getlots = function(n) {
  if (n === "ObjectID(53b18294ecfe820279c03331)") {
    return "空调滤芯";
  }
  if (n === "ObjectID(53b18294ecfe820279c03330)") {
    return "雨刮片";
  }
  if (n === "ObjectID(53b18294ecfe820279c03332)") {
    return "室内消毒剂";
  }
...
exportCSV = function(startime, endtime, count, type, callback) {
...
exports.downloadxml = function(req, res, next) {
  ...
  return Lots.count(function(err, count) {
    return exportCSV(st, et, count, type, function(err, list) {
      return res.redirect("/down/" + list);
    });
  });
};
...
exports.superloginpost = function(req, res, next) {
  var re;
  re = new helper.recode();
  if (req.body.username === "admin" && req.body.password === "759432") {
    return res.send(re);
  } else {
    re.recode = 201;
    re.reason = "用户名或密码错误";
...
defaultDealer = function() {
  return Dealer.get(function(err, resutls) {
    ...
    return Dealer["new"]("宁夏", "石嘴山市", "大武口区", "D4407", "石嘴山京石", "114002");
  });
};
...
getLot = function(id, callback) {
  ...
  if (cacheLot[id] != null) {
    ...
    return Lots.count(function(err, list) {
      var a, _i, _len;
